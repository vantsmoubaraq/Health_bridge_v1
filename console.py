...
class Console(cmd.Cmd):
    ...
    def preloop(self):
        print(Fore.LIGHTBLUE_EX + "\nHello and Welcome to HealthBridge" + Style.RESET_ALL + "  (default, March 12 2023, 08:01:47)" )
        print("\nA user-friendly and comprehensive Hospital Management System software solution that can address the needs of hospitals and improve their operations\n")
        print("Version 1.0\n")
        ...
        try:
            ...
            if len(arguments) == 1:
                ...
                if arguments[0] in classes:
                    ...
                    print(Fore.RED + "Please enter a valid class" + Style.RESET_ALL)
        except Exception:
            pass
    ...
    def do_create(self, args):
        ...
        try:
            ...
            if len(arguments) == 1:
                ...
                obj.save()
                print(Fore.MAGENTA + f"Successfully created {obj.__class__.__name__} object: id --  {obj.id}" + Style.RESET_ALL)
                return
            if len(arguments) > 1:
                new_dict = {}
                for entry in arguments[1:len(arguments)]:
                    ...
                    if attr == "True" or attr == "False":
                        attr = bool(attr)
                    elif attr[0] == '"' and attr[len(attr) - 1 ] == '"' or attr[0] == "'" and attr[len(attr) - 1] == "'":
                        attr = str(attr)
                        attr = attr.replace('"', "")
                        attr = attr.replace("'","")
                        attr = attr.replace("_", " ")
                ...
                obj.save()
                print(f"Successfully Created {obj.__class__.__name__} object: " + Fore.LIGHTGREEN_EX  + f" HealthBridge_id --  {obj.id}"  + Style.RESET_ALL)
        except Exception:
            pass
            ...
            if len(arguments) > 1:
                ...
                for key in models.storage.all().keys():
                    ...
                    if cid == obj_id:
                        ...
                        print(Fore.LIGHTMAGENTA_EX + "Instance Updated" + Style.RESET_ALL)
        except Exception:
            pass
    ...
    def do_count(self, args):
        ...
        try:
            if len(args) == 0:
                print("The total number of objects in Storage is {}".format(len(models.storage.all())))
                return
            arguments = args.split(" ")
            if len(arguments) == 1:
                if arguments[0] in classes:
                    obj_count = len(models.storage.all(arguments[0]))
                    print("The total number of {}s in the HealthBridge Hospital Management System is ".format(arguments[0]) + Fore.LIGHTGREEN_EX + "{}".format(obj_count) + Style.RESET_ALL)
                    return
                else:
                    print(Fore.RED + "Please enter a valid class" + Style.RESET_ALL)
        except Exception:
